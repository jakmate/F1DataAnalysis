...
def plot_std_dev_lap_times(stats, constructor_colors):
    ...
    plot_data = stats[['firstName', 'lastName', 'constructor', 'adjustedStd']].reset_index()
    plot_data = plot_data.melt(id_vars=['firstName', 'lastName', 'constructor'],
    ...
    _ = ax.bar(x, plot_data_sorted['Standard Deviation (s)'],
...
def plot_mean_lap_times(stats, constructor_colors):
    ...
    plot_data = stats[['firstName', 'lastName', 'constructor', 'adjustedMean']].reset_index()
    plot_data = plot_data.melt(id_vars=['firstName', 'lastName', 'constructor'],
    ...
    plot_data_sorted['DifFromFastest'] = (plot_data_sorted['Mean'] - plot_data_sorted['Mean'].min()) / 6000
    ...
    _, ax = plt.subplots(figsize=(16, 10))
    _ = ax.bar(x, plot_data_sorted['DifFromFastest'],
                  color=plot_data_sorted['constructor'].map(constructor_colors),
                  width=bar_width, align='center')
