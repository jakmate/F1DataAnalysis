...
class JolpicaF1API:
    ...
    def _write_cache(self, cache_path: Path, data: Dict[str, Any]):
        ...
        try:
            cache_obj = {"timestamp": datetime.now().isoformat(), "data": data}
    ...
    def get_qualifying_results(
        self, season: str = "current", round: int = 0
    ) -> Dict[str, Any]:
        return self._make_request(
            f"{season}/{round}/qualifying", {"season": season, "round": round}
        )
    ...
    def get_race_results(
        ...
        return self._make_request(
            f"{season}/{round}/results", {"season": season, "round": round}
        )
    ...
    def get_laps(self, season: str = "current", round: int = 0) -> Dict[str, Any]:
        ...
        try:
            ...
            num_drivers = (
                len(race_data["Laps"][0]["Timings"]) if race_data["Laps"] else 20
            )
        ...
        return {
            "MRData": {
                **init_response["MRData"],
                ...
                "RaceTable": {"Races": [{**race_data, "Laps": final_laps}]},
            }
        }
    ...
    def get_driver_standings(
        ...
        if round > 0:
            return self._make_request(
                f"{season}/{round}/driverstandings", {"season": season, "round": round}
            )
        else:
    ...
    def get_constructor_standings(
        ...
        if round > 0:
            return self._make_request(
                f"{season}/{round}/constructorstandings",
                {"season": season, "round": round},
            )
        else:
            return self._make_request(
                f"{season}/constructorstandings", {"season": season}
            )
    ...
    def get_pitstops(self, season: str = "current", round: int = 0) -> Dict[str, Any]:
        return self._make_request(
            f"{season}/{round}/pitstops", {"season": season, "round": round}
        )

    def get_race_info(self, season: str = "current", round: int = 0) -> Dict[str, Any]:
        if round > 0:
            return self._make_request(
                f"{season}/{round}/races", {"season": season, "round": round}
            )
        else:
    ...
    def _handle_rate_limit_error(self, response: requests.Response) -> float:
        if self.rate_limit_wait:
            retry_after = min(int(response.headers.get("Retry-After", 1)), 300)
